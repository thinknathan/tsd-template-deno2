...
export function on_message(
  ...
  if (message_id === hash("features")) {
    ...
    print("To get started simply use `deno run` and get going!");
    msg.post("#", "usage");
  } else if (message_id === hash("usage")) {
    print(
      "  ✔ Use `deno run dev` to start a watcher that compiles and emits lua and script when you save",
    );
    print("  ✔ Use `deno run build` to just compile your ts, sans watcher ");
